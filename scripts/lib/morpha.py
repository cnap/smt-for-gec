...
MorphaDir = os.environ('MORPH_HOME')
MorphaCmd = os.path.join(MorphaDir, 'morpha.pipe')
MorphaArgs= ['-a', '-f', os.path.join(MorphaDir, 'verbstem.list')]
...
def get_gen_pipe():
    ...
    if _genpipe is None:
        open_gen_pipe()
    elif _genpipe.returncode is not None:
        sys.stderr.write("Pipe seems to have died, restarting\n")
        open_gen_pipe()
    return _genpipe
...
def open_pipe():
    global _pipe
    sys.stderr.write("Opening morpha pipe\n")
